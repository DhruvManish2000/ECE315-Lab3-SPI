/*
* ECE - 315 : WINTER 2022
* LAB 3: Interfacing Using the Zynq-7000 SPI Interface - PART 1
*
*
...
* Students have to find the loop count values at which IDLE% is 85%, 75%, 65%, 55% and 45%.
...
static void cpuLoadGenTask( void *pvParameters );
static TaskHandle_t xTask_cpuload;

static void printRunTimeStatsTask( void *pvParameters );
static TaskHandle_t print_run_time_stats_task_handle;

static void loopCountProcessorTask(void *pvParameters);
static TaskHandle_t loop_count_processor_task_handle;
...
int main(void)
{
	xTaskCreate(loopCountProcessorTask,
				(const char *) "Loop_Count_Manager",
				configMINIMAL_STACK_SIZE*5,
				NULL,
				tskIDLE_PRIORITY + 1,
				&loop_count_processor_task_handle);

	xTaskCreate(printRunTimeStatsTask,
				(const char *) "Stats",
				configMINIMAL_STACK_SIZE*5,
				NULL,
				tskIDLE_PRIORITY + 1,
				&print_run_time_stats_task_handle);


	xTaskCreate(cpuLoadGenTask,
				( const char * ) "Load_Gen",
				configMINIMAL_STACK_SIZE*5,
				NULL,
				tskIDLE_PRIORITY,
...
static void loopCountProcessorTask(void *pvParameters){
	...
	TickType_t delay = pdMS_TO_TICKS(6000);

	for(;;){
		/*
		* Increment loop count by a specified amount of your choice (maybe increments of 25K or 50K or 75K)
		...
		* That means, in case if you choose a loop count increment higher 50000, you will have to provide more delay to let this loop count stabilize!
		...
		*/
		...
		/**************************************************/
		//write a line of code that increments the "loop_count" variable by 25000/50000/75000.

		loop_count = loop_count + 75000;
		/**************************************************/
...
static void cpuLoadGenTask( void *pvParameters ){
	...
	while(1){
		...
		*/
		...
		// XOR with all 1s 0 and 1 is 0 and 1 and 1 is 0 - suitable for what we are doing here :)
...
static void printRunTimeStatsTask( void *pvParameters )
{
	...
	while(1){
		...
		/**************************************************/
		/*
		* Call the vTaskGetRunTimeStats() function to find the load percentages of all running tasks
		* Print the results of the vTaskGetRunTimeStats() function onto the terminal
		* The output[] array variable that you want to use with the vTaskGetRunTimeStats() function is declared for you in this task.
		* Also, print the "run" and "loop_count" variable on the terminal.
		*/
