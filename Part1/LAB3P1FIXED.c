...
// Checks if  the transmit buffer is full then just stays in the loop forever, once its not full then of course proceed
				//while (XUartPs_IsTransmitFull(XPAR_XUARTPS_0_BASEADDR) == TRUE){};

// write function pending UART WRITE FUNCTION (JUST NEED TO FIND FROM DRIVER FILE)
	...
	while(1){
		...
		if(spi_master_loopback_en==1 && current_command_execution_flag==2) 				//just send the characters back to task 1. No SPI access! This is because loopback is currently enabled.
			...
			if(bytecount == TRANSFER_SIZE_IN_BYTES){	//byte is read from the FIFO1 and then it is transferred to the SPI connection
			    ...
			    xQueueSendToBack(xQueue_FIFO2, &send_SPI_data_via_FIFO2, portMAX_DELAY);
			    bytecount = 0;
...
static void TaskSpi0Peripheral( void *pvParameters ){
	...
	while(1){
		...
		if(spi_master_loopback_en==0 && current_command_execution_flag==2){
			...
			//SpiSlaveWrite(&fornow, TRANSFER_SIZE_IN_BYTES);
			if(temp_store != DOLLAR){
				if(end_sequence_flag == 2 && temp_store == CHAR_CARRIAGE_RETURN){
					message_counter++;
					flag = 1;
				...
				else if(end_sequence_flag == 1 && temp_store == CHAR_PERCENT){
					end_sequence_flag =2 ;
					num_received++;
					SpiSlaveWrite(&temp_store,TRANSFER_SIZE_IN_BYTES);
				}
